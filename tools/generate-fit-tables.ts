...
import { isArrayField } from "src/utils";
import { undocumented_messages, undocumented_types } from "src/undocumented";

console.log(
  `// GENERATED BY:\n`,
  `// % npm run generate-fit-tables\n`,
  "// Re-run this whenever you update @garmin/fitsdk\n\n"
);
...
const flipped_types = Object.fromEntries(
  Object.entries(FIT.types).map(
    ([key, value]) =>
      [
        key,
        Object.fromEntries(
          Object.entries(value).map(([k, v]) => {
            const num = Number(k);
            if (!k || isNaN(num) || num !== Math.round(num)) {
              throw new Error(
                `In type ${key}:${v} expected a number, but got "${k}"`
              );
            }
...
Object.entries(undocumented_types).forEach(([key, value]) => {
  if (!Object.prototype.hasOwnProperty.call(flipped_types, key)) {
    flipped_types[key] = {};
  }
  Object.entries(value).forEach(([k, v]) => (flipped_types[key][k] = v));
});

Object.entries(undocumented_messages).forEach(([key, value]) => {
  let msg =
    "value" in value
      ? FIT.messages[value.value]
      : Object.values(FIT.messages).find((v) => v.name === key);
  if (!msg) {
    if (!("value" in value)) {
      throw new Error(`Missing value in undocumented message ${key}`);
    }
    msg = FIT.messages[value.value] = {
      num: value.value,
      name: key,
      messagesKey: `${key}Mesgs`,
      fields: {},
    };
  }
  Object.values(value.fields).forEach(
    (v) => msg.fields[v.num] || (msg.fields[v.num] = v)
  );
});

const fit_types = Object.fromEntries(
  Object.entries(flipped_types)
    .concat([
      [
        "device_index",
        {
    ...
    .map(([key, value]) => {
      let max = -Infinity;
      let min = Infinity;
      const mapped = Object.entries(value).map(([k, num]) => {
        if (num < min) min = num;
        if (num > max) max = num;
