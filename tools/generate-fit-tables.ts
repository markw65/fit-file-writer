import { FIT as FITBase, FitField } from "fit-defs";
...
function mergeObjects<T, U extends DeepPartial<T>>(target: T, source: U) {
  ...
  Object.entries(s).forEach(([key, value]) => {
    if (
      ...
      mergeObjects(t[key], value);
    }
  });
  return target;
}

const FIT = mergeObjects(structuredClone(FITBase), {
  // fit-file-parser isn't up to date with all of garmin's fields.
  // we can add missing ones here instead.
  messages: {
    20: {
      87: {
